	...
	if(msg_arr[1] && msg_arr[1] != 'find') {
		...
		if(msg_arr[1].length > 0) {
			...
			var item = {
				...
				chat_id : msg.chat.id,
				num : msg_arr[1],
				main_info : msg.text.replace('/auto ', '').replace('/auto@AzbnBot ', ''),
		...
		if(msg_arr[2].length > 0) {
			...
			azbn.mdl('mysql').query("SELECT * FROM `" + azbn.mdl('cfg').dbt.autochecker + "` WHERE num LIKE '%" + msg_arr[2] + "%' ORDER BY created_at DESC LIMIT 50", function(err, rows, fields) {
